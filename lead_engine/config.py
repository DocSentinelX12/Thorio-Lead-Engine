...
class LeadEngineConfig:
    ...
    airtable_research_table: str = DEFAULT_AIRTABLE_RESEARCH_TABLE
    airtable_companies_table: str = DEFAULT_AIRTABLE_COMPANIES_TABLE
    airtable_opportunities_table: str = DEFAULT_AIRTABLE_OPPORTUNITIES_TABLE
    airtable_outreach_table: str = DEFAULT_AIRTABLE_OUTREACH_TABLE
    airtable_referrals_table: str = DEFAULT_AIRTABLE_REFERRALS_TABLE
    airtable_followups_table: str = DEFAULT_AIRTABLE_FOLLOWUPS_TABLE
    airtable_commissions_table: str = DEFAULT_AIRTABLE_COMMISSIONS_TABLE
    airtable_lead_sources_table: str = DEFAULT_AIRTABLE_LEAD_SOURCES_TABLE
    ...
    def from_environment(cls):
        database_dir = os.getenv(
            ...
            os.getenv("LEAD_ENGINE_DATABASE_DIR", "data"),
        )

        airtable_table = os.getenv(
            "AIRTABLE_LEAD_TABLE",
            os.getenv("AIRTABLE_TABLE", DEFAULT_AIRTABLE_LEAD_TABLE),
        )
        airtable_research_table = os.getenv(
            "AIRTABLE_RESEARCH_TABLE",
            DEFAULT_AIRTABLE_RESEARCH_TABLE,
        )
        airtable_companies_table = os.getenv(
            "AIRTABLE_COMPANIES_TABLE", DEFAULT_AIRTABLE_COMPANIES_TABLE
        )
        airtable_opportunities_table = os.getenv(
            "AIRTABLE_OPPORTUNITIES_TABLE", DEFAULT_AIRTABLE_OPPORTUNITIES_TABLE
        )
        airtable_outreach_table = os.getenv(
            "AIRTABLE_OUTREACH_TABLE", DEFAULT_AIRTABLE_OUTREACH_TABLE
        )
        airtable_referrals_table = os.getenv(
            "AIRTABLE_REFERRALS_TABLE", DEFAULT_AIRTABLE_REFERRALS_TABLE
        )
        airtable_followups_table = os.getenv(
            "AIRTABLE_FOLLOWUPS_TABLE", DEFAULT_AIRTABLE_FOLLOWUPS_TABLE
        )
        airtable_commissions_table = os.getenv(
            "AIRTABLE_COMMISSIONS_TABLE", DEFAULT_AIRTABLE_COMMISSIONS_TABLE
        )
        airtable_lead_sources_table = os.getenv(
            "AIRTABLE_LEAD_SOURCES_TABLE", DEFAULT_AIRTABLE_LEAD_SOURCES_TABLE
        )

        raw_batch_size = os.getenv("LEAD_ENGINE_BATCH_SIZE", str(DEFAULT_BATCH_SIZE))
        try:
        ...
        return cls(
            ...
            airtable_research_table=airtable_research_table,
            airtable_companies_table=airtable_companies_table,
            airtable_opportunities_table=airtable_opportunities_table,
            airtable_outreach_table=airtable_outreach_table,
            airtable_referrals_table=airtable_referrals_table,
            airtable_followups_table=airtable_followups_table,
            airtable_commissions_table=airtable_commissions_table,
    ...
    def airtable_tables(self) -> Dict[str, str]:
        return {
            ...
            "research": self.airtable_research_table,
            "companies": self.airtable_companies_table,
            "opportunities": self.airtable_opportunities_table,
            "outreach": self.airtable_outreach_table,
            "referrals": self.airtable_referrals_table,
            "followups": self.airtable_followups_table,
            "commissions": self.airtable_commissions_table,
            "lead_sources": self.airtable_lead_sources_table,
        }

    def validate(self) -> None:
        if not isinstance(self.database_dir, str) or not self.database_dir.strip():
            raise ValueError("database_dir must be a non-empty string.")
        ...
        expected = {
            "lead_radar", "research", "companies", "opportunities",
            "outreach", "referrals", "followups", "commissions", "lead_sources",
        }
        if set(required_tables) != expected:
            raise ValueError("Airtable table configuration does not match the required Master Tracker tables.")
        for table_key, table_name in required_tables.items():
            if not isinstance(table_name, str) or not table_name.strip():
                raise ValueError(f"Airtable table '{table_key}' must be a non-empty string.")
            if table_name != table_name.strip():
                raise ValueError(f"Airtable table '{table_key}' contains leading or trailing whitespace.")

    def safe_dict(self):
        return {
            "database_dir": self.database_dir,
            "airtable_configured": bool(self.airtable_base_id),
