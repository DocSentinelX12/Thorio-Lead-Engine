...
def _discovery_handler_for(agent: str, payload: Mapping[str, Any], ctx: AgentExecutionContext) -> Dict[str, Any]:
    """Run one permanent source specialist without qualifying the lead."""
    ...
    if agent != "web_job_signal":
        ...
        job_markers = (
            ...
            "workable", "ashby", "remote", "jobicy", "himalayas", "remote ok",
            "remotejobs", "arbeitnow", "muse",
        )
    ...
    if fingerprint and lead is not None:
        gated = apply_discovery_gate(_PipelineAdapter(ctx.db), {"fingerprint": fingerprint})
        lead = gated.get("lead") or lead
...
def _qualification_a(_: str, payload: Mapping[str, Any], ctx: AgentExecutionContext) -> Dict[str, Any]:
    ...
    # Deep company/contact research is a mandatory downstream work item.
    # The research specialist may only fill verified facts. It never invents
    # a decision maker, contact method, communication, or consent.
    enqueue(
        ctx.db,
        "company_research",
        {
    ...
    paxus_status = str(paxus.get("research_status") or "").strip().lower()
    if paxus.get("qualified") and not paxus.get("true_referral"):
        enqueue(
            ctx.db,
            "paxus_research",
            {"lead": evaluated, "paxus qualification": paxus},
            priority=10,
            dedupe_key=f"paxus_research:{fingerprint}",
        )
        paxus_status = "research_required"
    ...
    return {
        ...
        "handoffs": ["company_research", "qualification_b"] + (["paxus_research"] if paxus_status == "research_required" else []),
    }
...
def _qualification_b(_: str, payload: Mapping[str, Any], ctx: AgentExecutionContext) -> Dict[str, Any]:
    ...
    enqueue(
        ctx.db,
        "priority",
        {"lead": evaluated, "evidence_events": payload.get("evidence_events", [])},
        priority=5,
    ...
    return {
        ...
        "challenged_prior_result": payload.get("prior_result") is not None,
    }
...
def _company_research(_: str, payload: Mapping[str, Any], ctx: AgentExecutionContext) -> Dict[str, Any]:
    ...
    # This worker is deliberately evidence-only. A real research provider can
    # supply verified facts through the task payload. Without such evidence we
    # persist a durable research-required state rather than claiming research
    # occurred or fabricating a decision maker.
    ...
    research_complete = bool(
        supplied.get("company_verified")
        and supplied.get("decision_maker")
        and supplied.get("decision_maker_evidence")
    )
    ...
    return {
        ...
        "decision_maker_verified": bool(supplied.get("decision_maker") and supplied.get("decision_maker_evidence")),
        "fabricated_fields": [],
    }
...
def _paxus_research(_: str, payload: Mapping[str, Any], ctx: AgentExecutionContext) -> Dict[str, Any]:
    ...
    return {
        ...
        "lead": refreshed,
        "queue_result": result,
        "true_referral": bool(paxus.get("true_referral")),
        "research_status": refreshed.get("research_status", "research_required"),
    }
...
def _priority(_: str, payload: Mapping[str, Any], __: AgentExecutionContext) -> Dict[str, Any]:
    ...
    return {
        ...
        "actionability": "ready" if research_ready and decision_maker_ready else "research_required",
        "inputs_used": ["evidence", "qualification", "freshness", "research", "decision_maker"],
    }
