...
def _require_mapping(value: Any, label: str) -> Dict[str, Any]:
    if not isinstance(value, Mapping): raise AgentContractError(f"{label} must be a mapping")
    return dict(value)
def _lead_payload(payload: Mapping[str, Any]) -> Dict[str, Any]: return _require_mapping(payload.get("lead", payload), "lead")
...
def _discovery_handler_for(agent: str, payload: Mapping[str, Any], ctx: AgentExecutionContext) -> Dict[str, Any]:
    record = _require_mapping(payload.get("record", payload), "record"); source = _source_text(record); signal = str(record.get("signal") or record.get("evidence") or "").strip()
...
def _make_discovery_handler(agent: str) -> Callable[..., Dict[str, Any]]:
    def handler(_: str, payload: Mapping[str, Any], ctx: AgentExecutionContext) -> Dict[str, Any]: return _discovery_handler_for(agent, payload, ctx)
    handler.__name__ = f"_{agent}_handler"; return handler
...
def _company_research(_: str, payload: Mapping[str, Any], ctx: AgentExecutionContext) -> Dict[str, Any]:
    ...
    if payload_findings:
        specialist_findings = {**dict(specialist_findings), **dict(payload_findings)}
    canonical = build_canonical_research_package(lead, merged_research, specialist_findings)
    for section_name in RESEARCH_SECTIONS:
        current = lead.get(section_name)
        if isinstance(current, Mapping):
            status = str(current.get("verification_status") or current.get("status") or "").strip().lower()
            if current.get("verified") is True or status in {"verified", "research_verified", "complete"}:
                canonical[section_name] = dict(current)
            else:
                merged_section = dict(canonical[section_name]); merged_section.update(dict(current))
                canonical[section_name] = merged_section
    merged = dict(lead); merged["company_research"] = merged_research
    ...
    for field in RESEARCH_SECTIONS[:-1]:
        ...
        if isinstance(section, Mapping):
            ...
            if section.get("verified") is True or status in {"verified", "research_verified", "complete"}:
                verified_fields.append(field)
    for key, value in merged_research.items():
        if value not in (None, "", [], {}, ()) and key not in {"fabricated_fields", "observed_input", "social_findings", "social_evidence_sources", "social_evidence_count", "observed_decision_maker", "observed_decision_maker_evidence", "public_web_research", "public_web_sources", "public_company_facts", "public_product_facts", "public_hiring_facts", "public_decision_maker_facts", "public_business_need_facts", "public_commercial_facts", "researched_at"}:
    ...
    merged["research_status"] = status; merged["research_verified_fields"] = verified_fields
    stored = _persist_lead(ctx.db, merged)
    if status == "complete": enqueue(ctx.db, "qualification_a", {"lead": stored, "evidence_events": payload.get("evidence_events", []), "research_result": {"status": status, "verified_fields": verified_fields}}, priority=9, dedupe_key=f"qualification_a:{fingerprint}")
    return {"role": "company_research", "lead": stored, "research": merged_research, "research_status": status, "verified_fields": verified_fields, "canonical_sections": list(RESEARCH_SECTIONS), "decision_maker_verified": str(merged_research.get("decision_maker_verification_status") or "").lower() == "verified", "fabricated_fields": list(merged_research.get("fabricated_fields") or []), "handoff": "qualification_a" if status == "complete" else "research_required"}
...
def _duplicate_resolution(_: str, payload: Mapping[str, Any], __: AgentExecutionContext) -> Dict[str, Any]:
    lead = _lead_payload(payload); candidates = payload.get("candidates", [])
    if not isinstance(candidates, Iterable) or isinstance(candidates, (str, bytes, Mapping)): raise AgentContractError("candidates must be a list-like collection")
    return {"role": "duplicate_resolution", "lead": lead, "candidate_count": len([item for item in candidates if isinstance(item, Mapping)]), "decision": payload.get("decision", "requires_identity_comparison"), "preserve_distinct_opportunities": True}

def _priority(_: str, payload: Mapping[str, Any], ctx: AgentExecutionContext) -> Dict[str, Any]:
    lead = _lead_payload(payload); evidence = sum(1 for key in ("signal", "evidence", "job_title", "person", "company") if str(lead.get(key) or "").strip()); qualified = bool(lead.get("qualified") or lead.get("potential_routes")); freshness = bool(lead.get("need_at") or lead.get("current_need_at") or lead.get("inquiry_at") or lead.get("last_inquiry_at") or lead.get("intent_at")); research_ready = str(lead.get("research_status") or "").strip().lower() in {"complete", "research_complete"}; decision_maker_ready = bool(lead.get("company_research", {}).get("decision_maker")) if isinstance(lead.get("company_research"), Mapping) else False; score = evidence + (5 if qualified else 0) + (3 if freshness else 0) + (2 if research_ready else 0) + (1 if decision_maker_ready else 0); fingerprint = str(lead.get("fingerprint") or "").strip()
    if not fingerprint: raise AgentContractError("priority requires lead fingerprint")
...
def _outreach_closer(_: str, payload: Mapping[str, Any], ctx: AgentExecutionContext) -> Dict[str, Any]:
    ...
    if not research.get("decision_maker") or not research.get("decision_maker_evidence") or str(research.get("decision_maker_verification_status") or "").strip().lower() != "verified": raise AgentContractError("outreach_closer requires verified decision-maker research")
    try: decision = build_outreach_decision(lead)
...
def _follow_up(_: str, payload: Mapping[str, Any], ctx: AgentExecutionContext) -> Dict[str, Any]:
    lead = _lead_payload(payload); fingerprint = str(lead.get("fingerprint") or "").strip()
    if not fingerprint: raise AgentContractError("follow_up requires lead fingerprint")
    snapshot = dict(lead); current = ctx.db.get(fingerprint)
    if current is None: raise AgentContractError(f"follow_up lead not found: {fingerprint}")
    lead = dict(current); task_event_id = str(payload.get("inbound_event_id") or "").strip()
    if task_event_id:
        events = lead.get("conversation_events"); latest_event_id = str(events[-1].get("event_id") or "").strip() if isinstance(events, list) and events and isinstance(events[-1], Mapping) else ""
    ...
    updated = apply_outcome(lead, outcome); objection = str(payload.get("objection") or "").strip()
    if objection and outcome not in {"opted_out", "declined", "irrelevant", "converted", "exhausted"}: updated["outreach_objection"] = objection
    if updated.get("outreach_state") in {"declined", "opted_out", "irrelevant", "exhausted", "converted"}:
    ...
    if not route: raise AgentContractError("follow_up requires an active revenue route")
...
def _monitoring(_: str, payload: Mapping[str, Any], ctx: AgentExecutionContext) -> Dict[str, Any]:
    ...
    tasks = pending(ctx.db); return {"role": "monitoring", "queue_depth": len(tasks), "running_count": sum(1 for task in tasks if task.get("status") == "running"), "queued_count": sum(1 for task in tasks if task.get("status") == "queued"), "failed_count": sum(1 for task in tasks if task.get("status") == "failed")}
def _audit(_: str, payload: Mapping[str, Any], __: AgentExecutionContext) -> Dict[str, Any]:
    lead = _lead_payload(payload); qualification = lead.get("qualification_results") or {}; violations = []
    if lead.get("paxus_true_referral") and not qualification.get("Paxus", {}).get("true_referral"): violations.append("paxus_true_referral_without_qualification_evidence")
    if lead.get("qualified") and not lead.get("potential_routes") and not qualification: violations.append("global_qualified_without_route_evidence")
    if lead.get("research_status") == "complete":
        research = lead.get("company_research")
        if not isinstance(research, Mapping) or not research.get("company_verified") or not research.get("decision_maker") or not research.get("decision_maker_evidence") or str(research.get("decision_maker_verification_status") or "").strip().lower() != "verified": violations.append("research_complete_without_verified_research")
...
_PROCESSORS: Dict[str, Callable[..., Dict[str, Any]]] = {"qualification_a": _qualification_a, "qualification_b": _qualification_b, "company_research": _company_research, "paxus_research": _paxus_research, "identity_resolution": identity_resolution, "duplicate_resolution": _duplicate_resolution, "verification": verification, "priority": _priority, "routing": routing, "airtable_integrity": airtable_integrity, "outreach_closer": _outreach_closer, "follow_up": _follow_up, "monitoring": _monitoring, "audit": _audit}

def handler_registry() -> Dict[str, Callable[..., Dict[str, Any]]]:
    advanced = advanced_handler_registry()
    for protected in ("priority", "verification", "routing", "airtable_integrity", "outreach_closer", "follow_up"):
        advanced.pop(protected, None)
...
def _validate_specialization(agent: str, handler: Callable[..., Dict[str, Any]]) -> AgentSpecialization:
    ...
    if handler is None: raise AgentContractError(f"No handler registered for specialist {agent}")
