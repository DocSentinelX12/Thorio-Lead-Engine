...
def sync_commission(
    ...
    accidentally overwrite an existing paid commission state.
    """
    ...
    if "actual_commission" in lead:
        fields["Actual Amount"] = lead.get(
            "actual_commission"
        )

    if "commission_payment_date" in lead:
        fields["Payment Date"] = (
            _text(
                lead.get(
    ...
    if "commission_payment_method" in lead:
