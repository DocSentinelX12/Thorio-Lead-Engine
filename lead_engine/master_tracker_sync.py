...
def _routes(lead: Dict[str, Any]) -> List[str]:
    ...
    return list(dict.fromkeys(_text(route) for route in value if _text(route) in {"Paxus", "Shiftr", "Thorio", "Astrivon Labs"}))
...
def sync_astrivon_referral(lead: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    fields = {
        ...
        "Company": company,
        "Opportunity": _opportunity_key(lead, "Astrivon Labs"),
        "Partner": "Astrivon Labs",
        "Submitted Date": _text(referral.get("introduced_at") or lead.get("astrivon_introduced_at"))[:10] or None,
        "Partner Confirmed": referral.get("partner_confirmed") is True or lead.get("astrivon_partner_confirmed") is True,
        "Partner Confirmation / ID": _text(referral.get("referral_id") or lead.get("astrivon_referral_id")) or None,
        "Outcome": "Active" if referral.get("partner_confirmed") is True or lead.get("astrivon_partner_confirmed") is True else "Qualified",
...
def sync_astrivon_commissions(lead: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for event in events:
        ...
        fields = {
            "Company": company, "Referral": referral_key, "Partner": "Astrivon Labs",
            "Deal / Placement Value": revenue, "Commission Rate": 0.20,
            "Expected Commission": round(revenue * 0.20, 2),
            "Eligible / Trigger Date": _text(event.get("received_at"))[:10] or None,
            "Paid": event.get("paid") is True,
            "Actual Amount": event.get("actual_commission"),
            "Payment Date": _text(event.get("commission_payment_date"))[:10] or None,
            "Payment Method": _text(event.get("commission_payment_method")) or None,
            "Notes": _text(event.get("notes")) or None,
        }
