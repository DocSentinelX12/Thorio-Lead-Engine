...
def build_canonical_research_package(lead: Mapping[str, Any], company_research: Mapping[str, Any], specialist_findings: Mapping[str, Any] | None = None) -> Dict[str, Dict[str, Any]]:
    ...
    package["research_gaps"] = {"verified": False, "verification_status": "research_required" if missing else "observed_evidence", "researched_at": _now(), "missing_sections": missing, "unknowns": ["company_verification", "decision_maker_verification", "current_need_verification", "route_verification"], "provenance": {"source": "canonical_research_sections", "checked_sections": list(VERIFIABLE_RESEARCH_SECTIONS), "missing_count": len(missing)}}
