...
def _explicitly_verified(value: Any) -> bool:
    ...
    if status in {"verified", "research_verified", "complete"}:
        return True
    # route_research is intentionally a container for route-scoped verification.
    # A verified nested route is sufficient for that section, but only when that
    # nested route itself has evidence and an explicit verified state.
    routes = value.get("routes")
    if isinstance(routes, Mapping):
