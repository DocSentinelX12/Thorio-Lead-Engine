...
class SourceDefinition:
    ...
    def __post_init__(self) -> None:
        ...
        collector_type = self.collector_type.strip().lower()
        url = self.url.strip()
        pagination_type = self.pagination_type.strip().lower()
        ...
        if self.offset_step is not None and self.offset_step <= 0:
            raise ValueError(
        ...
        if self.attribution_required and not self.attribution_url:
            raise ValueError(
    ...
    def source_key(self) -> str:
        ...
        The provider is included so two collectors with the same
        display name cannot accidentally collide across providers.
        """
    ...
    def allows_thorio(self) -> bool:
        return self.enabled and self.allowed_for_thorio
