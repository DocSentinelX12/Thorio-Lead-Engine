...
for agent, source in _SOURCE_ROLES.items():
    _SPECIALIZATIONS.append(AgentSpecialization(agent, f"Discover high-value hiring and business-intent signals available through authorized {source} access.", ("detect relevant intent", "identify entities", "capture source evidence", "normalize signal metadata"), ("source evidence",), ("normalized signals", "evidence references"), ("unauthorized access", "credential automation", "qualification decisions", "invented evidence")))
for agent, target in _DISCOVERY_INTELLIGENCE.items():
    _SPECIALIZATIONS.append(AgentSpecialization(agent, f"Discover and consolidate {target} from already collected evidence without fabricating facts.", ("scan evidence", "detect target signal", "group related observations", "preserve provenance", "emit research gaps"), ("lead or evidence_events",), ("discovery findings", "evidence references", "research requirements"), ("invented evidence", "premature qualification", "silent evidence deletion")))
...
for agent, (mission, responsibilities) in _PROCESSING.items():
    ...
    if agent == "duplicate_resolution": forbidden = forbidden + ("merging distinct opportunities",)
    if agent == "monitoring": forbidden = forbidden + ("hiding failures",)
    if agent == "follow_up": forbidden = forbidden + ("initiating revenue outreach", "sending revenue messages", "handling sales conversations without closer authorization")
...
def get_specialization(agent: str) -> AgentSpecialization:
    ...
    except KeyError as exc: raise ValueError(f"No professional specialization defined for agent: {agent}") from exc
