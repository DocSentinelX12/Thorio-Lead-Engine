...
def test_outreach_worker_sends_autonomously_after_sales_eligibility(tmp_path):
    db = _db(tmp_path); now = _recent(); route_results = {"Thorio": {"qualified": True, "route_research": {"verified": True, "evidence": "Current software engineering hiring need."}}, "Shiftr": {"qualified": True, "route_research": {"verified": True, "evidence": "Current engineering support need."}}}; lead = {"fingerprint": "outreach-worker-test", "company": "Acme", "potential_routes": ["Thorio", "Shiftr"], "qualified": True, "sales_eligibility": "eligible", "signal": "Acme is hiring a remote software engineer", "research_status": "complete", "research_verified_fields": ["current_intent_research", "route_research"], "company_research": {"company_verified": True, "decision_maker": "Taylor", "decision_maker_evidence": "https://example.com/taylor", "contact_email": "taylor@example.com", "decision_maker_verification_status": "verified"}, "current_intent_research": {"verified": True, "verification_status": "verified", "current_need": "remote software engineer hiring", "observed_at": now, "evidence_url": "https://example.com/need"}, "route_research": {"verified": True, "verification_status": "verified", "routes": {"Thorio": {"verified": True, "verification_status": "verified", "evidence": "Current software engineering hiring need."}, "Shiftr": {"verified": True, "verification_status": "verified", "evidence": "Current engineering support need."}}}, "qualification_results": route_results, "evidence_events": [{"source_id": "evt-1", "source_url": "https://example.com/signal", "signal": "Acme is hiring a remote software engineer"}]}; db.insert_if_new(lead); enqueue(db, "outreach_closer", {"lead": lead}); transport = _FakeTransport(); register_revenue_transport(transport)
