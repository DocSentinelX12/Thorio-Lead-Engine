...
def _proof_debug(db, stored, drain, transport): return {"pending_agents": sorted({task.get("agent") for task in pending(db) if task.get("status") in {"queued", "running"}}), "pending_tasks": [{"agent": task.get("agent"), "status": task.get("status"), "error": task.get("last_error") or task.get("error")} for task in pending(db) if task.get("status") in {"queued", "running", "failed"}], "drain": drain, "sales_eligibility": stored.get("sales_eligibility"), "qualification_review_stage": stored.get("qualification_review_stage"), "verification_state": stored.get("verification_state"), "handoff": stored.get("handoff")}
