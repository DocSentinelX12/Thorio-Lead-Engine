...
def _proof_debug(db, stored, drain, transport): return {"failed_results": [{"agent": item.get("agent"), "claimed_count": item.get("claimed_count"), "completed_count": item.get("completed_count"), "failed_count": item.get("failed_count"), "results": item.get("results")} for round_data in drain.get("rounds", []) for item in round_data.get("agents", []) if int(item.get("failed_count", 0) or 0) > 0], "pending_verification": [{"id": task.get("id"), "dedupe_key": task.get("dedupe_key"), "payload": task.get("payload")} for task in pending(db) if task.get("agent") == "verification" and task.get("status") in {"queued", "running"}], "drain": drain, "sales_eligibility": stored.get("sales_eligibility"), "qualification_review_stage": stored.get("qualification_review_stage"), "verification_state": stored.get("verification_state"), "handoff": stored.get("handoff")}
