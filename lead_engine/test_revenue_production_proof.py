...
def _proof_debug(db, stored, drain, transport): return {"failed_tasks": [{"id": task.get("id"), "agent": task.get("agent"), "status": task.get("status"), "dedupe_key": task.get("dedupe_key"), "payload": task.get("payload"), "error": task.get("last_error") or task.get("error")} for task in pending(db) if task.get("status") == "failed"], "pending_verification": [{"id": task.get("id"), "dedupe_key": task.get("dedupe_key"), "payload": task.get("payload")} for task in pending(db) if task.get("agent") == "verification" and task.get("status") in {"queued", "running"}], "drain": drain, "sales_eligibility": stored.get("sales_eligibility"), "qualification_review_stage": stored.get("qualification_review_stage"), "verification_state": stored.get("verification_state"), "handoff": stored.get("handoff")}
