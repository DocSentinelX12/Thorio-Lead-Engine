...
def test_complete_production_revenue_lifecycle_has_no_orphaned_qualified_opportunity(tmp_path, monkeypatch):
    ...
    try:
        orchestrator = AgentOrchestrator(db, worker_prefix="production-proof"); orchestrator.dispatch_discovery("linkedin_signal", lead, priority=10); first = _drain(orchestrator); stored = db.get(lead["fingerprint"]); assert stored is not None; assert first["failed_count"] == 0, first; assert stored["qualified"] is True; assert stored.get("sales_eligibility") == "blocked", _proof_debug(db, stored, first, transport); assert transport.calls == []; db.record_airtable_handoff(lead["fingerprint"], package_digest(stored), "recLead", "recResearch", ["recCompany"], "2026-09-18T00:00:00+00:00"); db.mark_synced(lead["fingerprint"]); orchestrator.dispatch_processing("airtable_integrity", {"lead": stored, "routing_result": dict(stored["routing_result"])}, priority=6); second = _drain(orchestrator); stored = db.get(lead["fingerprint"]); assert second["failed_count"] == 0, second; assert stored.get("sales_eligibility") == "eligible", _proof_debug(db, stored, second, transport); assert stored["revenue_lifecycle_state"] == "outreach_sent", _proof_debug(db, stored, second, transport); assert stored["outreach_state"] == "awaiting_response"; assert len(transport.calls) == 1; assert not any(task.get("agent") == "outreach_closer" and task.get("status") == "queued" for task in pending(db))
