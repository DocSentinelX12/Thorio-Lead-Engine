...
def failover_after_exact_path_failure(
    ...
    *,
    ...
    if not isinstance(failed_path, Mapping):
        return {
            "migrate": False,
            "from_path_id": str(failed_path_id),
            "to_path_id": None,
            "reason": "failed_path_not_found",
        }
    ...
    independent = [
        ...
        and AdaptiveFabricRouteSelector.failure_domain_independent(
            failed_path,
            path_by_id[str(candidate["path_id"])],
        )
    ]
    if not independent:
        return {
    ...
    return {
        "migrate": True,
        "from_path_id": str(failed_path_id),
        "to_path_id": str(selected["path_id"]),
