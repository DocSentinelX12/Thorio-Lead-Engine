...
from compute_bridge import REMOTE_SAFE_AGENTS
...
def test_claim_creates_and_binds_physical_allocation_to_worker_node():
    with tempfile.TemporaryDirectory() as directory:
        ...
        coordinator = ComputeCoordinator(str(root / "coordinator.sqlite3"), auth_token="token", lease_seconds=30, inventory=inventory)
        coordinator.register_worker(_worker())
        task_id = coordinator.enqueue({"kind": "agent_task", "agent": "engineering_demand_discovery", "payload": {"lead": {"fingerprint": "lead-1"}, "evidence_events": [{"signal": "Hiring a backend engineer"}], "compute_requirements": {"workload_class": "cpu_bound", "min_cpu_count": 1, "min_memory_bytes": 1}}})
...
def test_completion_releases_physical_allocation():
    with tempfile.TemporaryDirectory() as directory:
        ...
        coordinator = ComputeCoordinator(str(root / "coordinator.sqlite3"), auth_token="token", lease_seconds=30, inventory=inventory)
        coordinator.register_worker(_worker())
        task_id = coordinator.enqueue({"kind": "agent_task", "agent": "engineering_demand_discovery", "payload": {"compute_requirements": {"workload_class": "cpu_bound", "min_cpu_count": 1, "min_memory_bytes": 1}}})
        claimed = coordinator.claim("worker-1")
        assert claimed is not None
...
def test_claim_without_physical_capacity_never_leases_work():
    with tempfile.TemporaryDirectory() as directory:
        ...
        coordinator = ComputeCoordinator(str(root / "coordinator.sqlite3"), auth_token="token", lease_seconds=30, inventory=inventory)
        coordinator.register_worker(_worker())
        task_id = coordinator.enqueue({"kind": "agent_task", "agent": "engineering_demand_discovery", "payload": {"compute_requirements": {"workload_class": "gpu_required", "gpu": {"gpu_count": 1}}}})
