...
def test_physical_path_measurement_timestamp_is_durable_and_survives_reload(tmp_path):
    ...
    inventory.persist_physical_verification(Verification(), evidence={"measurement": {"bandwidth_gbps": 180.0}}, observed_at=200.0)
...
def test_route_health_exposes_observed_performance_change_without_threshold_policy(tmp_path):
    ...
    for observed_at, bandwidth, latency in ((100.0, 500.0, 2.0), (200.0, 400.0, 3.0), (300.0, 350.0, 5.0)):
        verification = FabricVerificationResult(path_id=path.path_id, state=FabricPathState.MEASURED, measurement={"bandwidth_gbps": bandwidth, "latency_us": latency, "sample_count": 8}, measurement_observed_at=observed_at, required_segments=path.segments)
        inventory.persist_physical_verification(verification, observed_at=observed_at)
    health = inventory.physical_fabric_route_health(path_id=path.path_id)
    assert health["path_id"] == path.path_id
    assert health["observation_count"] == 3
    assert health["latest"]["observed_at"] == 300.0
    assert health["latest"]["measurement"]["latency_us"] == 5.0
    assert health["change"]["bandwidth_gbps"] == -150.0
    assert health["change"]["latency_us"] == 3.0
